...
namespace CFMStats.Classes
{
    public class MyFractions
    {
        ...
        //private MyFractions()
        //{
        //http://www.codeproject.com/Tips/623477/Convert-Decimal-to-Fraction-and-Vice-Versa-in-Csha
        //}

        public static string Convert(decimal pvalue, bool skip_rounding = true, decimal dplaces = (decimal)(0.0625))
        {
            dplaces = System.Convert.ToDecimal(0.0625);

            decimal value = pvalue;

            if (!skip_rounding)
            {
                value = DecimalRound(pvalue, dplaces);
            }
            ...
            if (mFraction > 0m)
            {
                ...
                uint intFractLength = System.Convert.ToUInt32(strFraction.Length);

                // set the numerator to have the proper amount of zeros
                mNumerator = System.Convert.ToUInt32(Math.Pow(10, intFractLength));
        ...
        public static decimal Convert(string value)
        {
            ...
            if (fparse.Count() == 2)
            {
                try
                {
                    decimal d0 = System.Convert.ToDecimal(fparse[0]);
                    // convert numerator
                    decimal d1 = System.Convert.ToDecimal(fparse[1]);
                    // convert denominator
                    dReturn = d0 / d1;
                    // divide the fraction (converts to decimal)
                    decimal dWhole = System.Convert.ToDecimal(whole);
                    // convert 
        ...
        public static bool ReformatForFeetAndInches(ref string line_type, bool zero_for_blank = true)
        {
            ...
            s1 = d1 == 0 ? "" : d1.ToString() + "'";
            s2 = d2 == 0 ? "" : MyFractions.Convert(d2) + "\"";
        ...
        {
            string sPlaces = MyFractions.Convert(places, true);
