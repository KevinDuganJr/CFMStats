...
CREATE PROCEDURE [dbo].[StatsDefenseByWeek_select] @stageIndex INT, 
										@playerId   INT, 
										@leagueId   INT
AS
	BEGIN
	   ...
	   SELECT 
			...
			(s.weekIndex) AS 'Week', 
			(defCatchAllowed) AS 'CatchesAllowed', 
			(defDeflections) AS 'Deflections', 
			(defForcedFum) AS 'ForcedFumble', 
			(defFumRec) AS 'FumbleRecovery', 
			(defIntReturnYds) AS 'INTReturnYards', 
			(defInts) AS 'Interceptions', 
			(CONVERT(DECIMAL(10, 2), defSacks)) AS 'Sacks', 
			(defSafeties) AS 'Safety', 
